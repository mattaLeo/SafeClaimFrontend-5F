...
export class CreaPolizzaComponent implements OnInit {
  ...
  formData: Partial<Polizza> = { tipo_copertura: 'RCA' };
  loading        = false;
  errorMessage   = '';
  successMessage = '';
  ...
  submit(): void {
    ...
    const assicuratoreId = Number(this.auth.currentUser?.id);

    if (!assicuratoreId) {
      this.errorMessage = 'Utente non riconosciuto. Rieffettua il login.';
      return;
    }
